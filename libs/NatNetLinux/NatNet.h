...
public:
    ...
    std::string asString(){
        ...
        ss
        <<std::fixed
        << std::setprecision(3) << qx << ", "
        << std::setprecision(3) << qy << ", "
        << std::setprecision(3) << qz << ", "
        << std::setprecision(3) << qw << " )" << std::endl;
        return ss.str();
    }
...
public:
    ...
    std::string asString(){
        std::stringstream ss;
        ss
        <<"    Rigid Body: "<<id()<<std::endl
        <<"        loc: "<<location().asString()
        <<"        ori: "<<orientation().asString()<<std::endl;
        return ss.str();
    }
...
public:
    ...
    std::string asString(){
        ...
        std::stringstream ss;
        ss
        <<"    MarkerSet: '"<<name()<<"'"<<std::endl;
        size = _markers.size();
        for(i=0;i<size;++i){
            ss<<"        "<<_markers[i].asString();
        }
        ss<<std::endl;
        return ss.str();
    }
...
public:
    ...
    std::string asCompactString(){
        std::stringstream ss;
        ss<<"Frame# "<<frameNum()<<", "<<_uidMarker.size()<<" uMarkers"<<std::endl;
        return ss.str();
    }
    ...
    std::string asString(){
        ...
        ss
        <<"--Frame--"<<std::endl
        <<"  Frame#: "<<frameNum()<<std::endl;
        size = _markerSet.size();
        ss
        <<"  MarkerSets: "<<size<<std::endl;
        for(i=0;i<size;++i){
            ss<<_markerSet[i].asString();
        }
        size = _uidMarker.size();
        ss<<"Unidentified Markers: "<<size<<std::endl;
        size = _rBodies.size();
        ss
        <<"  RigidBodies: "<<size<<std::endl;
        for(i=0;i<size;++i){
            ss<<_rBodies[i].asString();
        }
        int hour,min,sec,fframe,subframe;
        timecode(hour,min,sec,fframe,subframe);
